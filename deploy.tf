...
resource "null_resource" "deploy_cockroachdb_vols" {
    depends_on = ["null_resource.setup_kubectl"]
    provisioner "local-exec" {
        command = <<EOF
            until kubectl get pods 2>/dev/null; do printf '.'; sleep 5; done
            kubectl create -f ./examples/cockroachdb/volumes.yaml

EOF
    }
}

resource "null_resource" "deploy_cockroachdb" {
    depends_on = ["null_resource.deploy_cockroachdb_vols"]
    provisioner "local-exec" {
        command = <<EOF
            until kubectl get pods 2>/dev/null; do printf '.'; sleep 5; done
            kubectl create -f ./examples/cockroachdb/statefulset.yaml
...
/*
resource "null_resource" "deploy_nodejs_helloworld" {
    depends_on = ["null_resource.setup_kubectl"]
    provisioner "local-exec" {
        command = <<EOF
